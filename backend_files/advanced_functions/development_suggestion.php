...
function get_not_attending($event_id, $user_id, $connection)
{
	$sql = "SELECT facebook_id FROM user
		WHERE user.facebook_id <> ALL (
				SELECT DISTINCT IsAttending.facebook_id as id
				FROM event, IsAttending
				WHERE event.id = IsAttending.event_id
				AND facebook_id <> '$user_id')";
...
foreach($event_ids as $event_id)
{
	...
	$users_not_attending = get_not_attending($event_id, $facebook_id, $conn);
